...
**Hallucination** — LLMs sometimes generate plausible but incorrect outputs. An LLM might suggest activating an actuator that does not exist, or claim a sensor is showing a value it is not. Mitigations include constraining the output to a JSON schema (structured output mode) so the agent can only produce valid actuator IDs, validating all tool call arguments before execution, and never trusting an LLM's claim about sensor values — always reading from the ground truth (the database or the API).

**Latency** — a cloud LLM API call takes 500–3,000 ms, too slow for any decision on the critical control path. Mitigations include using LLMs only for deliberative reasoning (decisions that can tolerate seconds of latency), using ML models for real-time inference, caching common reasoning patterns, and using smaller, faster local models for routine decisions.

**Cost** — calling a cloud LLM API for every sensor reading is economically unsustainable. A building with 100 sensors reading every 5 seconds would make 1.7 million LLM calls per day. Mitigations include invoking the LLM only when there is a novel situation or anomaly rather than for routine operation, and using the ML model as a filter that escalates to the LLM only when needed.
...
The interaction pattern is: the system defines a set of tools (functions) with names, descriptions, and parameter schemas; the user or system sends a message to the LLM along with the tool definitions; the LLM reasons about what to do and, if it needs to act, generates a **tool call**, a structured JSON specifying the function name and arguments; the calling code executes the function and gets the result; the result is returned to the LLM as a **tool result** message; the LLM reasons about the result and either calls another tool or produces a final response.
...
Good tool design for building control follows several principles.

**One tool per concern.** Rather than a single `control_building()` tool that does everything, the architecture provides `read_sensors()`, `set_actuator()`, `find_route()`, `highlight_rooms()`, `query_history()`, `create_alert()`, each with a clear, narrow purpose.
...
**Validate inputs before execution.** Tool call arguments from the LLM should never be trusted unconditionally. Validating that `room_id` exists, that `sensor_type` is a valid type, and that numeric arguments are in valid ranges, then returning an error message if validation fails, allows the LLM to retry with corrected arguments.

**Return rich, informative results.** The LLM reasons about tool results. Instead of returning just `{"state": "on"}`, returning `{"state": "on", "last_changed": "2025-03-15T14:32:05Z", "commanded_by": "safety_agent"}` gives the LLM additional context for better decisions.
...
The architecture has three components: the **MCP Host** (the application that runs the LLM, such as Claude Code or an AI agent application), the **MCP Client** (the protocol client inside the host that manages server connections), and the **MCP Server** (a server that exposes tools, resources, and prompts via the MCP protocol).

For building control, an **MCP server** would wrap the BuildSim REST API endpoints as MCP tools. Any MCP-compatible agent (Claude, GPT-4, a local Ollama model) can then discover and use those tools automatically, without any custom integration code.
...
MCP uses JSON-RPC 2.0 over stdio or HTTP with Server-Sent Events for streaming. Implementing an MCP server for BuildSim means defining tool schemas for each BuildSim endpoint, implementing the tool handler functions (which call the BuildSim REST API), exposing a `tools/list` endpoint that returns the tool definitions, and exposing a `tools/call` endpoint that executes a tool call.

The [MCP Python SDK](https://github.com/modelcontextprotocol/python-sdk) handles the protocol; only the tool implementations need to be written. See the [MCP specification](https://spec.modelcontextprotocol.io/) for the full protocol.

## Agent Frameworks

### Choosing a Framework

Agent frameworks provide scaffolding for building LLM agents: tool execution loops, memory management, state machines, and multi-agent coordination. The right choice depends on the complexity of the use case.
...
**[LangChain](https://python.langchain.com/)** — the most widely used LLM application framework. Provides chains (sequential tool calls), agents (LLM-driven tool selection), memory (conversation history), and a large ecosystem of integrations. Best for applications that need many integrations quickly.
...
**[CrewAI](https://crewai.com/)** — a framework for role-based multi-agent systems. Agents are defined with roles, goals, and backstories; tasks have descriptions and expected outputs; crews assign tasks to agents. Agents can delegate to each other. Good for workflows that map naturally to human team structures. [CrewAI documentation](https://docs.crewai.com/).
...
The key insight is that the reasoning (Thought) is explicit and can be logged and audited. This is critical for safety-critical systems: the exact reasoning behind each decision is visible.

## Multi-Agent Coordination

### Why Multiple Agents?

A single all-knowing agent that controls every aspect of a building is fragile and hard to test. Multiple specialised agents, each focused on a single objective, offer several advantages.

**Separation of concerns.** A safety agent is responsible for fire and emergency response. An energy agent optimises consumption. A comfort agent maintains occupant preferences. Each agent has clear responsibilities and can be developed, tested, and updated independently.
...
Multi-agent systems create the possibility of **conflict**: the energy agent wants to reduce HVAC power consumption, but the comfort agent wants to increase it to maintain temperature.

**Priority-based:** a fixed priority is assigned to each agent. Safety always overrides energy; comfort overrides energy. When two agents issue conflicting commands, the higher-priority agent wins. Simple and predictable, but inflexible.
...
**Shared context in LLM:** for LLM-based agents, the entire conversation history (including other agents' decisions) can be included in each agent's context. This allows passive awareness without direct communication, as each agent sees what others have decided. Expensive in token cost but produces well-informed decisions.
...
## Safety and Trust in Agentic AI
...
**Guardrails** are constraints applied before an action is executed. Examples include: never locking all exits simultaneously, even if the LLM reasons that it should; smoke suppression can only be deactivated by a human operator; actuator commands above a certain rate are rejected and flagged for human review; if any action would result in all HVAC units being off simultaneously in winter, human confirmation is required.
...
**Audit trail** — every agent decision, tool call, and reasoning step must be logged immutably. When something goes wrong, reconstructing exactly what the agent decided and why is essential. The ReAct pattern makes this natural: every Thought, Action, and Observation is logged.

**Graceful degradation** — when the AI agent is unavailable, when the LLM API is unreachable, or when the agent crashes, the system must fall back to a safe state without human intervention. All actuators remain in their current state, rule-based fallbacks handle the most critical safety functions, alerts notify operators that AI control is inactive, and the system can be manually operated via the BuildSim API directly.

**Testing safety** — adversarial testing is essential: injecting sensor faults, simulating network outages, sending the agent into novel situations it has never seen. Does the agent behave safely when the smoke sensor reads 0 for 10 minutes? When two conflicting commands arrive simultaneously? When the LLM API returns an error? Tests for these scenarios should be written before deployment.
